	...
	<div class="col-md-3">
		<div class="box box-primary">	
			...
			<div class="box-footer">
					<div id="refresh-org">
					<?php
						$user_org = App\Modules\Chishiki\Http\Models\helpdesk\Agent_panel_panel\User_org::where('user_id','=', $users->id)->first();
					?>
						@if($user_org == null)
		...
		<?php 	$org_id = $user_org->org_id;
				$organization = App\Modules\Chishiki\Http\Models\helpdesk\Agent_panel_panel\Organization::where('id','=',$org_id)->first(); ?>
						<b>{!! Lang::get('lang.organization') !!}</b>
...
<?php $open = count(App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status', '=','1')->get());
$counted = count(App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status', '=','2')->get());
$deleted = count(App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status', '=','5')->get()); ?>
          <div class="row">
            <div class="col-md-12">
              <!-- Custom Tabs -->
              <div class="nav-tabs-custom">
                ...
                <div class="tab-content">
                  <div class="tab-pane active" id="tab_1">
                    {{-- open tab --}}
						    ...
						    {!! Form::open(['route'=>'select_all','method'=>'post']) !!}
						        <div class="mailbox-controls">
						            ...
						            <div class="pull-right">
						                <?php
						$counted = count(App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status', '=','1')->get());
						if ($counted < 20) {
						...
						?>
						        ...
						        <!-- table -->
						            <table class="table table-hover table-striped">
						                ...
						                <tbody id="hello">
						                    <?php $tickets = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status', '=','1')->orderBy('id', 'DESC')->paginate(20);?>
						...
						?> >
						                        ...
						                        <?php 
						                        //  collaborators
						                        $collaborators = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Ticket_Collaborator::where('ticket_id','=',$ticket->id)->get();
						                        $collab = count($collaborators);
						                        //  title
						                        $title = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Ticket_Thread::where('ticket_id', '=', $ticket->id)->first();
						                        $string = strip_tags($title->title);
						                        // check atatchments
						                        $attachments = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Ticket_attachments::where('thread_id','=',$title->id)->first();
						                        ...
						                        $TicketData = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Ticket_Thread::where('ticket_id', '=', $ticket->id)->max('id');
						                        ...
						                        $titles = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Ticket_Thread::where('ticket_id', '=', $ticket->id)->get();
						                        $count = count($titles);
						                        ...
						                        <?php $priority = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Ticket_Priority::where('priority_id', '=', $ticket->priority_id)->first();?>
                  ...
                  <div class="tab-pane" id="tab_2">
                    {{-- open tab --}}
						    <?php $closed = count(App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status', 2)->get()); ?>
						        @if(Session::has('success'))
						        <div id="success-alert" class="alert alert-success alert-dismissable">
						            <i class="fa  fa-check-circle"> </i> <b> Success </b>
						            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
						...
						?>
						        ...
						        <!-- table -->
						            <table class="table table-hover table-striped">
						                ...
						                <tbody id="hello">
						                    <?php $tickets = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status', '=','2')->orderBy('id', 'DESC')->paginate(20);?>
						                    @foreach ($tickets  as $ticket)
						                    <tr <?php if ($ticket->seen_by == null) {?> style="color:green;" <?php } ?> >
						                        <td ><input type="checkbox" class="icheckbox_flat-blue" name="select_all[]" value="{{$ticket->id}}"/></td>
						                        <?php 
                  ...
                  <div class="tab-pane" id="tab_3">
                    {{-- open tab --}}
						    <?php $deleted = count(App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status', '=','5')->get()); ?>
						    ...
						    {!! Form::open(['route'=>'select_all','method'=>'post']) !!}
						        <div class="mailbox-controls">
						            ...
						            <div class="pull-right">
						                <?php
						$counted = count(App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status', '=','5')->get());
						if ($counted < 20) {
							echo $counted . "/" . $counted;
						} else {
							echo "20/" . $counted;
						...
						?>
						        ...
						        <!-- table -->
						            <table class="table table-hover table-striped">
						                ...
						                <tbody id="hello">
						                    <?php $tickets = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status', '=','5')->orderBy('id', 'DESC')->paginate(20);?>
						                    @foreach ($tickets  as $ticket)
						                    <tr <?php if ($ticket->seen_by == null) {?> style="color:green;" <?php }
						?> >
						                        <td ><input type="checkbox" class="icheckbox_flat-blue" name="select_all[]" value="{{$ticket->id}}"/></td>
...
<?php
$orgs = App\Modules\Chishiki\Http\Models\helpdesk\Agent_panel_panel\Organization::all();
?>
