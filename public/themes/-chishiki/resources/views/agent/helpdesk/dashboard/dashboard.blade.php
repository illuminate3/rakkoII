...
{{-- <script src="{{asset("lb-faveo/dist/js/bootstrap-datetimepicker4.7.14.min.js")}}" type="text/javascript"></script> --}}
    ...
    {!! Form::text('start_date',null,['class'=>'form-control','id'=>'datepicker4'])!!}
        ...
        <?php 
        $start_date = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('id','=','1')->first();
        if($start_date != null) {
...
<?php $departments = App\Modules\Chishiki\Http\Models\helpdesk\Agent_panel\Department::all(); ?>
@foreach($departments as $department)
<?php
$open = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('dept_id','=',$department->id)->where('status','=',1)->count(); 
$resolve = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('dept_id','=',$department->id)->where('status','=',2)->count(); 
$close = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('dept_id','=',$department->id)->where('status','=',3)->count(); 
$delete = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('dept_id','=',$department->id)->where('status','=',5)->count(); 
?>
