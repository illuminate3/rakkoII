...
<div id="content" class="site-content col-md-9">
    <div class="row">
        <?php $categories = App\Modules\Chishiki\Http\Models\kb\Category::all();
        ?>
        @foreach($categorys as $category)
        {{-- get the article_id where category_id == current category --}}
        <?php
        $all = App\Modules\Chishiki\Http\Models\kb\Relationship::all();
        /* from whole attribute pick the article_id */
        $page = App\Modules\Chishiki\Http\Models\kb\Relationship::where('category_id', '=',$category->id)->paginate('3');
        /* from whole attribute pick the article_id */
            ...
            <section class="box-categories">
                ...
                <ul class="fa-ul">
                    <?php foreach ($article_id as $id) {
                                 //$format = App\Modules\Chishiki\Http\Models\helpdesk\Settings\System::where('id','1')->first()->date_time_format;
                        ...
                        $article = App\Modules\Chishiki\Http\Models\kb\Article::where('id', '=', $id)->where('status', '=','1')->where('type', '=','1')->where('publish_time','<',$date)->get();
                        ?>
                    ...
                    <?php $str = $arti->description;
                        ...
                        $excerpt = App\Modules\Chishiki\Http\Controllers\Client\kb\UserController::getExcerpt($str, $startPos = 0, $maxLength = 50); ?>
...
<ul class="nav nav-pills nav-stacked nav-categories">
    @foreach($categorys as $category)
<?php
$num = \App\Modules\Chishiki\Http\Models\kb\Relationship::where('category_id','=', $category->id)->get();
