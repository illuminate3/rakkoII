...
<?php
 $inbox = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::all();
	...
	?>
     <?php $myticket = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('assigned_to', Auth::user()->id)->where('status','1')->get();?>
     <?php $unassigned = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('assigned_to', '0')->where('status','1')->get();
...
?>
                                        ...
                                        <li>
                                            <a href="{{url('trash')}}">
                                                ...
                                                <?php $deleted = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('status', '5')->get();?>
...
$open = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('status','=','1')->where('dept_id','=',$dept->id)->get();
$open = count($open);
// dd($open);

$closed = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('status','=','2'||'3')->where('dept_id','=',$dept->id)->get();
$closed = count($closed);
// dd($closed);

$underprocess = App\Modules\Chishiki\Http\Models\helpdesk\Ticket\Tickets::where('status','=','2'||'3')->where('assigned_to','=', '0')->get();
...
$group = App\Modules\Chishiki\Http\Models\helpdesk\Agent_panel\Groups::where('name', '=', $agent_group)->where('group_status', '=', '1')->first();
// dd($group);
?>
