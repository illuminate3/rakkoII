...
<html>
    <head>
        <meta charset="UTF-8">
        <?php
        $title = App\Modules\Support\Http\Models\HelpDesk\Settings\System::where('id','=','1')->first();
        if(isset($title->name)){
	...
	<div id="page" class="hfeed site">
            <header id="masthead" class="site-header" role="banner">
                ...
                <?php
                $company = App\Modules\Support\Http\Models\HelpDesk\Settings\Company::where('id', '=', '1')->first();
				$system = App\Modules\Support\Http\Models\HelpDesk\Settings\System::where('id', '=', '1')->first();
				?>
				@if($system->url)
				...
				<div id="navbar" class="navbar-wrapper text-center">
					<nav class="navbar navbar-default site-navigation" role="navigation">
						<ul class="nav navbar-nav navbar-menu">
							<li @yield('home')><a href="{{url('/')}}">{!! trans('kotoba::helpdesk.home') !!}</a></li>
							@if($system->first()->status == 1)
								<li @yield('submit')><a href="{{URL::route('form')}}">{!! trans('kotoba::helpdesk.submit_a_ticket') !!}</a></li>
							@endif
							<li @yield('kb')><a href="{!! url('knowledgebase') !!}">{!! trans('kotoba::helpdesk.knowledge_base') !!}</a>
                                <ul class="dropdown-menu">
                                	<li><a href="{{route('category-list')}}">{!! trans('kotoba::helpdesk.categories') !!}</a></li>
                                    <li><a href="{{route('article-list')}}">{!! trans('kotoba::helpdesk.articles') !!}</a></li>
                                </ul>
                            </li>
                            <?php $pages = App\Modules\Support\Http\Models\KB\Page::where('status', '1')->where('visibility', '1')->get();
                                ?>
                                @foreach($pages as $page)
                                    <li><a href="{{route('pages',$page->slug)}}">{{$page->name}}</a></li>
                                @endforeach
                        @if(Auth::user())
							<li @yield('myticket')><a href="{{url('mytickets')}}">{!! trans('kotoba::helpdesk.my_tickets') !!}</a></li>

                            {{-- <li @yield('contact')><a href="{{route('contact')}}">Contact us</a></li> --}}
							<li @yield('profile')><a href="#" >{!! trans('kotoba::helpdesk.my_profile') !!}</a>
								<ul class="dropdown-menu">
									<li>
                                    ...
                                    <h3 class="banner-title text-info h4">{{Auth::user()->first_name." ".Auth::user()->last_name}}</h3>
											<div class="banner-content">
												{{-- <a href="{{url('kb/client-profile')}}" class="btn btn-custom btn-xs">Edit Profile</a> --}} <a href="{{url('auth/logout')}}" class="btn btn-custom btn-xs">{!! trans('kotoba::helpdesk.log_out') !!}</a>
											</div>
											@if(Auth::user())
												@if(Auth::user()->role != 'user')
											<div class="banner-content">
												<a href="{{url('dashboard')}}" class="btn btn-custom btn-xs">{!! trans('kotoba::helpdesk.dashboard') !!}</a>
											</div>
												@endif
											@endif
											@if(Auth::user())
												@if(Auth::user()->role == 'user')
											<div class="banner-content">
												<a href="{{url('client-profile')}}" class="btn btn-custom btn-xs">{!! trans('kotoba::helpdesk.profile') !!}</a>
											</div>
												@endif
											@endif
						...
						<ul class="nav navbar-nav navbar-login">
							<li <?php if($errors->first('email') || $errors->first('password')){ ?> class="sfHover" <?php } else { ?> <?php  } ?> ><a href="#"  data-toggle="collapse"  <?php if($errors->first('email') || $errors->first('password')){ } else { ?> class="collapsed" <?php  } ?> data-target="#login-form">{!! trans('kotoba::helpdesk.login') !!} <i class="sub-indicator fa fa-chevron-circle-down fa-fw text-muted"></i></a></li>
						</ul><!-- .navbar-login -->
						<div id="login-form" <?php if($errors->first('email') || $errors->first('password')){ ?> class="login-form collapse fade clearfix in" <?php } else { ?> class="login-form collapse fade clearfix" <?php  } ?> >
					            ...
					            @if(Session::has('errors'))
									<div class="alert alert-danger alert-dismissable">
   										<!-- <i class="fa fa-ban"> </i> <b> {!! trans('kotoba::helpdesk.alert') !!}! </b> -->
   										<!-- <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> -->
   										@if(Session::has('error'))
   											{!! Session::get('error') !!}
   										@else
   											{!! trans('kotoba::helpdesk.required-error') !!}
   										@endif
									</div>
								@endif
								<div class="form-group has-feedback {{ $errors->has('email') ? 'has-error' : '' }}">
									{!! Form::text('email',null,['placeholder'=>trans('kotoba::helpdesk.e-mail'),'class' => 'form-control']) !!}
									<!-- {!! $errors->first('email', '<spam class="help-block">:message</spam>') !!} -->
									<span class="glyphicon glyphicon-envelope form-control-feedback"></span>
					          	</div>

								<div class="form-group has-feedback {{ $errors->has('password') ? 'has-error' : '' }}">
						            {!! Form::password('password',['placeholder'=>trans('kotoba::helpdesk.password'),'class' => 'form-control']) !!}
									<!-- {!! $errors->first('password', '<spam class="help-block">:message</spam>') !!} -->
						            <span class="glyphicon glyphicon-lock form-control-feedback"></span>
					          	</div>
								<ul class="list-unstyled pull-left">
									<li><a href="{{url('password/email')}}">{!! trans('kotoba::helpdesk.forgot_password') !!}</a><br></li>
									<li><a href="{{url('auth/register')}}">{!! trans('kotoba::helpdesk.create_account') !!}</a></li>
								</ul>
								<button type="submit" class="btn btn-custom pull-right">{!! trans('kotoba::helpdesk.login') !!}</button>
							{!! Form::close() !!}
						</div><!-- #login-form -->
                        @endif
					</nav><!-- #site-navigation -->
                            ...
                            <div class="form-border">
                                ...
                                <div class="form-inline ">
                                    <div class="form-group">
                                        <input type="text" name="s" class="search-field form-control input-lg" title="Enter search term" placeholder="{!! trans('kotoba::helpdesk.have_a_question?_type_your_search_term_here') !!}" />
                                    </div>
                                    <button type="submit" class="search-submit btn btn-custom btn-lg pull-right">{!! trans('kotoba::helpdesk.search') !!}</button>
                                </div>
                            </div>
                            {!! Form::close() !!}
                        </div>
...
<!-- /.content-wrapper -->
<?php
$footer1 = App\Modules\Support\Http\Models\HelpDesk\Theme\Widgets::where('name','=','footer1')->first();
$footer2 = App\Modules\Support\Http\Models\HelpDesk\Theme\Widgets::where('name','=','footer2')->first();
$footer3 = App\Modules\Support\Http\Models\HelpDesk\Theme\Widgets::where('name','=','footer3')->first();
$footer4 = App\Modules\Support\Http\Models\HelpDesk\Theme\Widgets::where('name','=','footer4')->first();
?>
			...
			<div class="clearfix"></div>
                    ...
                    <div class="row">
						<div class="site-info col-md-6">
                            <p class="text-muted">{!! trans('kotoba::helpdesk.copyright') !!} &copy; {!! date('Y') !!}  <a href="{!! $company->website !!}" target="_blank">{!! $company->company_name !!}</a>. {!! trans('kotoba::helpdesk.all_rights_reserved') !!}. {!! trans('kotoba::helpdesk.powered_by') !!} <a href="http://www.faveohelpdesk.com/"  target="_blank">Faveo</a></p>
                        </div>
                        <div class="site-social text-right col-md-6">
                             <?php $socials = App\Modules\Support\Http\Models\HelpDesk\Theme\Widgets::all(); ?>
